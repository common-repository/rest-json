<?php
/**
 * Plugin Name: WP-Restify
 * Description: JSON-based REST API for WordPress.
 * Version: 1.0.1
 ...
 */
include_once( dirname( __FILE__ ) . '/include/class-jsonserializable.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-datetime.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-responsehandler.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-server.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-responseinterface.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-response.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-posts.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-users.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-comments.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-customposttype.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-pages.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-media.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-taxonomies.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-meta.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-meta-posts.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-menu1.php' );
include_once( dirname( __FILE__ ) . '/include/class-wp-json-menu2.php' );
 ...
 */
function mssjson_api_init() {
	mssjson_api_register_rewrites();

	global $wp;
	$wp->add_query_var( 'json_route' );
}
add_action( 'init', 'mssjson_api_init' );

/**
 * Add rewrite rules.
 */
function mssjson_api_register_rewrites() {
 ...
 */
function mssjson_api_maybe_flush_rewrites() {
...
add_action( 'init', 'mssjson_api_maybe_flush_rewrites', 999 );
 ...
 *
 * @global WP_JSON_Menus      $wp_json_menus
 ...
 */
function mssjson_api_default_filters( $server ) {
	global $wp_json_menus, $wp_json_posts, $wp_json_pages, $wp_json_media, $wp_json_taxonomies ;
	...
	add_action( 'deprecated_function_run',           'mssjson_handle_deprecated_function', 10, 3 );
	add_filter( 'deprecated_function_trigger_error', '__return_false'                         );
	add_action( 'deprecated_argument_run',           'mssjson_handle_deprecated_argument', 10, 3 );
	add_filter( 'deprecated_argument_trigger_error', '__return_false'                         );

	// Default serving
	add_filter( 'json_serve_request', 'mssjson_send_cors_headers'             );
	add_filter( 'json_pre_dispatch',  'mssjson_handle_options_request', 10, 2 );
}
add_action( 'wp_json_server_before_serve', 'mssjson_api_default_filters', 10, 1 );
...
add_action( 'template_redirect', 'mssjson_api_loaded', -100 );
 ...
 */
function mssjson_api_activation( $network_wide ) {
...
register_activation_hook( __FILE__, 'mssjson_api_activation' );
 ...
 * @param bool $network_wide ?
 */
function mssjson_api_deactivation( $network_wide ) {
	if ( function_exists( 'is_multisite' ) && is_multisite() && $network_wide ) {
...
register_deactivation_hook( __FILE__, 'mssjson_api_deactivation' );
 ...
 */
function mssjson_register_scripts() {
	wp_register_script( 'wp-api', esc_url_raw( plugins_url( 'js/wp-api.js', __FILE__ ) ), array( 'jquery', 'backbone', 'underscore' ), '1.1', true );

	$settings = array( 'root' => esc_url_raw( get_json_url() ), 'nonce' => wp_create_nonce( 'wp_json' ) );
	wp_localize_script( 'wp-api', 'WP_API_Settings', $settings );
}
add_action( 'wp_enqueue_scripts', 'mssjson_register_scripts', -100 );
add_action( 'admin_enqueue_scripts', 'mssjson_register_scripts', -100 );

/**
 * Add the API URL to the WP RSD endpoint.
 */
function mssjson_output_rsd() {
	?>
	<api name="WP-API" blogID="1" preferred="false" apiLink="<?php echo get_json_url() ?>" />
	<?php
}
add_action( 'xmlrpc_rsd_apis', 'mssjson_output_rsd' );
 ...
 * @see get_json_url()
 */
function mssjson_output_link_wp_head() {
	...
	echo "<link rel='' href='" . esc_url( $api_root ) . "' />\n";
}
add_action( 'wp_head', 'mssjson_output_link_wp_head', 10, 0 );

/**
 * Send a Link header for the API.
 */
function mssjson_output_link_header() {
	if ( headers_sent() ) {
		return;
	}
	...
	header( 'Link: <' . $api_root . '>; rel=""', false );

	
}
add_action( 'template_redirect', 'mssjson_output_link_header', 11, 0 );
 ...
 */
function mssjson_register_post_type( $post_type, $args ) {
...
add_action( 'registered_post_type', 'mssjson_register_post_type', 10, 2 );
 ...
 *                             otherwise true.
 */
function mssjson_cookie_check_errors( $result ) {
	if ( ! empty( $result ) ) {
		return $result;
	}
...
add_filter( 'json_authentication_errors', 'mssjson_cookie_check_errors', 100 );

/**
 * Collect cookie authentication status.
 *
 * Collects errors from {@see wp_validate_auth_cookie} for
 * use by {@see mssjson_cookie_check_errors}.
 *
 * @see current_action()
 * @global mixed $wp_json_auth_cookie
 */
function mssjson_cookie_collect_status() {
	global $wp_json_auth_cookie;
...
add_action( 'auth_cookie_malformed',    'mssjson_cookie_collect_status' );
add_action( 'auth_cookie_expired',      'mssjson_cookie_collect_status' );
add_action( 'auth_cookie_bad_username', 'mssjson_cookie_collect_status' );
add_action( 'auth_cookie_bad_hash',     'mssjson_cookie_collect_status' );
add_action( 'auth_cookie_valid',        'mssjson_cookie_collect_status' );
 ...
 *                                            instance otherwise.
 */
function mssjson_ensure_response( $response ) {
 ...
 */
function mssjson_parse_date( $date, $force_utc = false ) {
	if ( $force_utc ) {
 ...
 *                    null on failure.
 */
function mssjson_get_date_with_gmt( $date, $force_utc = false ) {
	$date = mssjson_parse_date( $date, $force_utc );
 ...
 */
function mssjson_handle_deprecated_function( $function, $replacement, $version ) {
 ...
 * @param string $version     Version.
 */
function mssjson_handle_deprecated_argument( $function, $message, $version ) {
	if ( ! empty( $message ) ) {
 ...
 */
function mssjson_send_cors_headers( $value ) {
 ...
 */
function mssjson_handle_options_request( $response, $handler ) {
...
if ( ! function_exists( 'mssjson_last_error_msg' ) ):
/**
 ...
 */
function mssjson_last_error_msg() {
